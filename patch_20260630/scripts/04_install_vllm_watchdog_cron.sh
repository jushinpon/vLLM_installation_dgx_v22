...
# Purpose:
#   Install the vLLM generation watchdog on cluster195 master node.
#   The watchdog probes node13 locally through SSH and runs a real
#   /v1/chat/completions request. After 3 consecutive failures, it restarts
#   only the vLLM backend through the existing manager.
#
# Run on:
#   cluster195 master node, as root.

TS="$(date +%Y%m%d_%H%M%S)"
BACKUP_DIR="/root/codex_backups_vllm_watchdog/$TS"
WATCHDOG="/usr/local/sbin/vllm_qwen35b_watchdog.sh"
CRON_FILE="/etc/cron.d/vllm_qwen35b_watchdog"
LOGROTATE_FILE="/etc/logrotate.d/vllm_qwen35b_watchdog"

mkdir -p "$BACKUP_DIR"

for f in "$WATCHDOG" "$CRON_FILE" "$LOGROTATE_FILE"; do
  if [ -f "$f" ]; then
    cp -a "$f" "$BACKUP_DIR/"
  fi
done
...
echo "===RUN_ONCE==="
"$WATCHDOG"
tail -40 /var/log/vllm_qwen35b_watchdog.log
