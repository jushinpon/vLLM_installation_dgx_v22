...
Model: `Qwen-Qwen3.6-35B-A3B-FP8` (default, FP8 quantized dense model).
...
Their deployment defaults now point to Qwen3.6-35B-A3B-FP8.
...
the nginx gateway and management scripts, and an **Ubuntu** DGX Spark backend
node that runs vLLM. The scripts are distro-agnostic on the backend side —
`install_vllm-v022.sh` and `deploy_vllm4dgx_v022_qwen35b.pl` use only Python
and standard Linux commands, no `yum` or `apt`. The nginx gateway installer
(`deploy_nginx_gateway_v022_qwen35b.pl`) uses `yum` and runs only on the Rocky
Linux master.

**Master node (Rocky Linux) prerequisites:**

```bash
# Install Perl modules and nginx
sudo dnf install -y nginx perl-JSON-PP perl-File-Path perl-Digest-SHA perl-HTTP-Tiny

# Ensure passwordless SSH to the backend
ssh-copy-id root@<backend-hostname>
```

**Backend node (Ubuntu DGX Spark) prerequisites:**
...
# Ensure /local_opt has enough disk space (model weights ~60GB + install ~20GB)
df -h /local_opt
```

**Deployment steps (run from master):**

```bash
# 1. Clone this repo on the master
git clone <repo-url> /home/vLLM_installation_dgx_v22
cd /home/vLLM_installation_dgx_v22

# 2. Full install (copies repo to backend, builds vLLM, downloads model, deploys)
bash bootstrap_new_cluster_v022_qwen35b.sh --full-install \
  --backend-host=<backend-hostname>
...
Replace `<backend-hostname>` with the backend's hostname or IP (e.g. `node13`,
`dgx-spark`, or an IP address). The master must be able to SSH to the backend
...
- Model: Qwen3.6-35B-A3B-FP8 (FP8 quantized dense model)
